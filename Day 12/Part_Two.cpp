...
{
    if(mp[i][j][left] != -1) return mp[i][j][left];
...
void solve()
{
    mp.resize(200, vector<vector<int>>(200, vector<int> (200, -1)));
    ...
    {
        new_str += "?" + str;
        for(auto i : arr) new_arr.push_back(i);   
    }

    new_str = "." + new_str;
    part_2_ans += fun(0, new_str, 0, new_arr, 0);
